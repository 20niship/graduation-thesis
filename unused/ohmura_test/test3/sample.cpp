...
int main() {
	...
	while(!prog_finish){
	  ...
	  double ref[6]={joint_angles1[0], 0 , 0 , 0, 0, 0};

#if 1	  
	  if(ref[0] - joint_angles2[0]>0.04)
	    ref[0]=joint_angles2[0]+0.04;
	  else if(ref[0]-joint_angles2[0]<-0.04)
	    ref[0]=joint_angles2[0]-0.04;
#endif
	  
	  hr4capi_set_joint_reference(instance_no2,ref , mask);

	  //hr4capi_set_joint_trajectory(instance_no2,ref , (double)stime/1000, 1, mask);
