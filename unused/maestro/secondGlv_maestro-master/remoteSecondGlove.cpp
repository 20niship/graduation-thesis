...
#include "mmcpplib.h"
#include "remoteSecondGlove.h"		// Application header file.
...
{
	int a_id = atoi(argv[1]) -1;	// debug number
	if (argc != 2){
		cout << "Set Debug number" << endl;
	}
	cout << a_id << endl;

	MainInit(); //	Initialize system, axes and all needed initializations

	// TO DO ... Set Power on axis
	cout << "can you power on ??" << endl;

	powerOnAxis(a1);
	powerOnAxis(a2);
	powerOnAxis(a3);

	cout << "End power on" << endl;
	// Start while loop
	MachineSequences(); //	Execute the state machine to handle the system sequences and control

	// TO DO ... Set Power OFF axis
	powerOffAxis(a1);
	powerOffAxis(a2);
	powerOffAxis(a3);
...
void ReadAllInputData()
{
	...
	control_a2.set_CommandFromHost(mbus_read_out.regArr, eCommand2);
	control_a3.set_CommandFromHost(mbus_read_out.regArr, eCommand3);
	return;
}
...
void WriteAllOutputData()
{
	...
	write_axis_info_to_mbus(a2, eAx2);
	write_axis_info_to_mbus(a3, eAx3);
//	// Write orders
//	write_TorControlVals_to_mbus(control_a1, eAx1 + 6);
//	write_TorControlVals_to_mbus(control_a2, eAx2 + 6);
//	write_TorControlVals_to_mbus(control_a3, eAx3 + 6);
//	write_TorControlVals_to_mbus(control_a4, eAx4 + 6);
//	write_TorControlVals_to_mbus(control_a5, eAx5 + 6);
//	write_TorControlVals_to_mbus(control_a6, eAx6 + 6);
