...
#include <h4rc/AxisInterface.hpp>

AxisInterface::AxisInterface(int mbusRefId, int gNumerator, int gDenominator, int mbusSendCmdId, int mbusTimeId) {
...
AxisInterface::~AxisInterface() { modbus_close(mb); }
...
void AxisInterface::send_cmd(float rad_edata, int Torlim) {
  if(enc0_slave_rad_is_loaded) {
    PosCmd = int(direction_mode_plus_or_minus * (4095. * (rad_edata - enc0_slave_rad) / (2.0 * M_PI)) * gRatio + pos0_motorInpt); // [cnt]
    ...
    cout << "/ and result signal" << direction_mode_plus_or_minus * (4095. * (rad_edata - enc0_slave_rad) / (2.0 * M_PI)) * gRatio;
...
int AxisInterface::get_pos() { return pos; }

int AxisInterface::get_vel() { return vel; }

int AxisInterface::get_cur() { return cur; }

int AxisInterface::get_PosCmd() { return PosCmd; }

double AxisInterface::get_motorOtptAxis_rad() { return (pos - pos0_motorInpt) * (2.0 * M_PI / 4095.) / gRatio + enc0_slave_rad; }
...
int AxisInterface::get_VelOrder() { return VelOrder; }

int AxisInterface::get_TorOrder() { return TorOrder; }
